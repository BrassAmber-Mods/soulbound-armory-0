...
public class ExperienceBarOverlay extends ScalableWidget {
    ...
    public void render(MatrixStack matrixes, int mouseX, int mouseY, float delta) {
        if (colors.alpha > 3) {
            ...
            if (this.component.canLevelUp()) {
                this.widthLimit(Math.min(1, this.component.statistic(experience).floatValue() / this.component.nextLevelXP()));
            }
    ...
    private void render(MatrixStack stack, int x, int y, int width) {
        if (colors.alpha > 3) {
            ...
            this.v(style.v + 5).widthLimit(this.component.canLevelUp() ? Math.min(1, this.component.statistic(experience).floatValue() / this.component.nextLevelXP()) : width).render(stack);
