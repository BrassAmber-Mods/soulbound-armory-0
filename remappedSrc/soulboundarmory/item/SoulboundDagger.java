...
public class SoulboundDagger extends SoulboundMeleeWeapon {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        DaggerStorage component = Components.weapon.of(player).storage(StorageType.dagger);
    ...
    public void onStoppedUsing(ItemStack itemStack, World world, LivingEntity entity, int timeLeft) {
        ...
        if (!world.isClient) {
            F attackSpeed = (float) component.attributeTotal(StatisticType.attackSpeed);
            F velocity = this.getMaxUsageRatio(attackSpeed, timeLeft) * attackSpeed;
            F maxVelocity = velocity / attackSpeed;
