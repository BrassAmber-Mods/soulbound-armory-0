...
public abstract class Widget<T extends Widget<T>> extends CellElement {
    ...
    public void renderForeground(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.text != null) {
            drawTextWithShadow(matrices, textRenderer, this.text, this.middleX() - textRenderer.getWidth(this.text) / 2, this.y() + this.height() / 2 - textRenderer.fontHeight / 2, this.active ? 0xFFFFFFFF : 0xA0FFFFFF);
        }
    }
