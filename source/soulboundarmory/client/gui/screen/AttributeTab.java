...
public class AttributeTab extends SoulboundTab {
    ...
    public void init() {
        ...
        resetButton.active = this.parent.item.statistics.get(attribute).values().stream().anyMatch(Statistic::aboveMin);
        ...
        for (var index = 0; index < size; index++) {
            ...
            add.active = this.parent.item.intValue(StatisticType.attributePoints) > 0 && attribute.belowMax();
        }
    }
    ...
    protected PressCallback<ScalableWidget> addPointAction(Statistic statistic) {
        return button -> Packets.serverAttribute.send(new ExtendedPacketBuffer(this.parent.item)
            .writeIdentifier(statistic.type.id())
            .writeInt(hasShiftDown() ? Integer.MAX_VALUE : 1)
        );
    }
    ...
    protected PressCallback<ScalableWidget> removePointAction(Statistic statistic) {
        return button -> Packets.serverAttribute.send(new ExtendedPacketBuffer(this.parent.item)
            .writeIdentifier(statistic.type.id())
            .writeInt(hasShiftDown() ? Integer.MIN_VALUE : -1)
        );
    }
}
